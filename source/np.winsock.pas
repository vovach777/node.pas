...
function CancelIoEx(hFile:THANDLE; lpOverlapped:Pointer):Boolean;stdcall;
...
begin
  if hDll = 0 then begin
    raise EIdWinsockStubError.Build(WSANOTINITIALISED, RSWinsockCallError, [string(AName)]);
  end;
  Result := Windows.GetProcAddress(hDll, PAnsiChar(AName));
  if Result = nil then begin
    raise EIdWinsockStubError.Build(WSAEINVAL, RSWinsockCallError, [string(AName)]);
  end;
end;
...
begin
  if cmsg = nil then begin
    ...
    if (UINT_PTR(cmsg) + WSA_CMSGHDR_ALIGN(cmsg^.cmsg_len) + SIZE_WSACMSGHDR) > (UINT_PTR(msg^.Control.buf) + msg^.Control.len) then begin
      Result := nil;
    end else begin
      Result := LPWSACMSGHDR(UINT_PTR(cmsg) + WSA_CMSGHDR_ALIGN(cmsg^.cmsg_len));
    end;
  end;
end;
...
{$IFDEF USE_INLINE}inline;{$ENDIF}
begin
  Result := WSA_CMSGDATA_ALIGN(UINT_PTR(SIZE_WSACMSGHDR + WSA_CMSGHDR_ALIGN(Alength)));
end;
...
function CancelIoEx; external 'Kernel32.dll';
